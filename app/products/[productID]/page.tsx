...
export default async function page(props: { params: any }) {
  ...
  const data = await fetch(
    `https://depot-data.onrender.com/products/${productId}`
  );
  ...
  return (
    <>
      <div className="wrapper">
        <div className="grid md:grid-cols-2 grid-cols-1 gap-20 my-8 ">
          ...
          <div>
            ...
            <div className="text-sm">
              ...
              <div>
                Categories:
                <span className="text-hover"> {product.category}</span>
              </div>
            </div>
          </div>
        </div>
        <div>
          <Details
            titleDetails={product.title}
            descriptionDetails={product.description}
            ratingDetails={product.rating.rate}
          />
        </div>
        <hr />
        <div>
          <p
            className="text-hover text-sm font-bold"
