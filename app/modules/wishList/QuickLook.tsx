...
export default function QuickLook({ product }: { product: ProductsTypes }) {
    ...
    return (
        <>
            ...
            <button onClick={() => handleOpen()} className={`bg-black text-xs text-white py-2 px-6 h-full hover:bg-[#a5a5a5] transition ease-in-out duration-300 font-semibold ${lang == "en" && "tracking-widest"}`}>{quickLook}</button>
        </>
    );
}