...
export default function QuickLook({ product }: { product: ProductsTypes }) {
  const { addToWishlist, quickLook } = useSelector(
  ...
  return (
    <>
      <div
        ...
        className="fixed w-[100vw] h-[100vh] left-0 top-0 justify-center items-center bg-[#000000a6] z-[1000] "
      >
          ...
          <div className=" flex flex-row flex-wrap p-8">
            ...
            <div className="md:w-[50%] w-full">
              <div className="text-black text-xl flex flex-col gap-2 ">
                <h1 className=" font-bold" style={{ letterSpacing: "1px" }}>
                  {product.title}
                </h1>
                <p>${product.price}</p>
              </div>
              ...
              <button className="flex gap-2 mt-8 text-hover hover:text-hover">
                <IconHeart color="black" size={20} />
                {addToWishlist}
              </button>
            </div>
          </div>
      ...
      <button
        onClick={() => handleOpen()}
        className={`bg-black text-xs text-white py-2 px-6 h-full hover:bg-[#a5a5a5] transition ease-in-out duration-300 font-semibold ${
