...
export default function AddToWishlist({ product }: propsType) {
  const { name, isLoggedIn, id } = useSelector(
    (state: RootState) => state.user
  );
  const { addToWishlist } = useSelector(
  ...
  useEffect(() => {
    const handelWishList = async () => {
      try {
        ...
        const foundElement = data.wishList.find(
          (item: ProductsTypes) => item.id === product.id
        );
        if (foundElement) {
          setInList(true);
        }
  ...
  return (
    <>
      {loading ? (
        ...
        <Link
          className="flex flex-row items-center gap-4 mt-4 mb-14"
          href="/wishList"
        >
